...
class GP:
	...
	def update(user_id, t):

		# maintain some data structures

		self.run_times[user_id] += 1
		self.run_time[user_id][run_times[user_id]] = t
		...
		tmp3=[]
		for i in range(self.run_times[u]):
			y_tmp.append(self.y[run_time[u][i+1]])
			m_tmp.append(self.prior_mean[u][self.a[u][self.run_time[u][i+1]]])
			tmp3.append(self.y[run_time[u][i+1]] - self.prior_mean[u][self.a[u][self.run_time[u][i+1]]])
		for i in range(self.model):
			for j in range(self.run_times[u]):
			
				self.sigma_t_k[u][j] = self.kernel[u][self.a[u][self.run_time[u][j+1]]][i]
		
			tmp1 = np.dot(self.sigma_t_k[u][:self.run_times[u]],c_t)			
...
def compute_prior(nTrain, nModel, train, sigma):
